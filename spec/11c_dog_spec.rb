...
describe Dog do
  ...
  subject(:anju) { described_class.new('Anju', 'German Shepherd', 'tan') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog and Cat both respond to :talk' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(anju).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(anju).to be_sleeping
  end
end
