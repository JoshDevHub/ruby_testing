...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continues) { described_class.new(5, '7') }
      # remove the 'x' before running this test
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(6, '6', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        slow_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_three.final_message }.to output(slow_phrase).to_stdout
      end
    end
  end
end
